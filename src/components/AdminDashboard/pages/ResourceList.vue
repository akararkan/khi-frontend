<template>
  <div class="rlist" dir="rtl">
    <!-- Silent redirect state -->
    <div v-if="isRedirecting" class="loading-card">
      <div class="spinner"></div>
      <div class="loading-text">پەڕەی ڕاست دەکرێتەوە...</div>
    </div>

    <!-- Generic fallback (only for resources that truly don't have custom page yet) -->
    <template v-else>
...
// ✅ ALL dedicated routes — redirect from generic to dedicated pages
const customListRouteNames = {
  ...
  albums: 'AdminAlbumList',  // ✅ Added: albums now has a dedicated route
}
...
watch(
  ...
  async () => {
    ...
    // Only redirect when this generic helper is actually active
...
.btn--primary{
  background:#8C1515;
  color:#fff;
  box-shadow:0 10px 24px rgba(140,21,21,.18);
}
.btn--ghost{
  background:#fff;
  color:#6E6B66;
