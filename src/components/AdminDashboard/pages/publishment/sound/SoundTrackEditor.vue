    ...
    <form @submit.prevent="submit" novalidate>
      <div class="ste__layout">
          ...
          <section class="card">
            ...
            <div v-if="topicMode === 'new'" style="margin-top:.9rem">
              ...
              <div class="topic-hint">
                ...
                موضوعی نوێ دروست دەکرێت و بە ئەم دەنگەوە دابمەزرێت
              </div>
        ...
        <aside class="col-side">
          ...
          <!-- Images (3 covers) -->
          <section class="card">
            <div class="card__hd"><span class="card__hd-ico">🖼</span> وێنەکان</div>
            ...
            <div class="img-block">
              ...
              <div class="field" style="margin-top:.75rem;">
                ...
                <input v-model="form.coverUrlCkb" class="inp" placeholder="https://…" :disabled="!!ckbCoverFile" />
              </div>
            </div>
            ...
            <div class="img-block">
              ...
              <div class="field" style="margin-top:.75rem;">
                <label class="lbl">یان URL بنووسە</label>
                <input v-model="form.coverUrlKmr" class="inp" placeholder="https://…" :disabled="!!kmrCoverFile" />
              </div>
            </div>
            ...
            <div class="img-block">
              ...
              <div class="field" style="margin-top:.75rem;">
                <label class="lbl">یان URL بنووسە</label>
                <input v-model="form.hoverUrl" class="inp" placeholder="https://…" :disabled="!!hoverFile" />
              </div>
            </div>
          </section>
          ...
          <section class="card" v-if="form.trackState === 'MULTI'">
            ...
            <div class="album-status" :class="form.albumOfMemories ? 'album-status--memories' : 'album-status--regular'">
              ...
              <div class="album-status__text">
                <span class="album-status__label">{{ form.albumOfMemories ? 'ئەلبوومی بیرەوەرییەکان' : 'ئەلبوومی ئاسایی' }}</span>
                <span class="album-status__hint">{{ form.albumOfMemories ? 'کۆمەڵە دەنگی بیرەوەری' : 'کۆمەڵە دەنگی ئاسایی' }}</span>
              </div>
            </div>
          </section>
...
const fetchTopics = async () => {
  ...
  try {
    const { data } = await api.get('/api/v1/soundtracks/topics')
...
const makeFileItem = (useUrl = false) => ({
  ...
  file: null, fileUrl: '', externalUrl: '', embedUrl: '',
...
const submit = async () => {
  ...
  try {
    ...
    const msg = e?.response?.data?.message || e?.response?.data?.error || e.message || 'هەڵەیەک ڕوویدا'
    showToast('error', msg)
