  ...
  <div class="alist" dir="rtl">
    ...
    <div class="alist__bar">
      <div class="search">
        ...
        <input v-model="searchQ" class="search__input" placeholder="گەڕان — ناونیشان، سلەگ (CKB/KMR)…" />
      ...
      <table class="tbl">
        <thead>
          <tr>
            ...
            <th>ناونیشان</th>
            <th>سلەگەکان</th>
            <th style="width:180px">ژێرناونیشان</th>
            <th style="width:80px">بلۆک</th>
        ...
        <tbody>
          <tr v-for="a in visibleItems" :key="a.id" class="tbl__row" @click="openDetail(a)">
            ...
            <td>
              <div class="tbl__name">{{ displayTitle(a) || '—' }}</div>
              <div class="tbl__name-alt" v-if="altTitle(a)">{{ altTitle(a) }}</div>
            </td>
            <td>
              <!-- Both slugs always visible in the table -->
              <div class="slug-stack">
                <span class="slug-pill slug-pill--ckb" v-if="a.slugCkb">
                  <span class="slug-pill__badge">CKB</span>{{ a.slugCkb }}
                </span>
                <span class="slug-pill slug-pill--kmr" v-if="a.slugKmr">
                  <span class="slug-pill__badge">KMR</span>{{ a.slugKmr }}
                </span>
                <span v-if="!a.slugCkb && !a.slugKmr" class="tbl__dash">—</span>
              </div>
            </td>
            <td><div class="tbl__sub-text">{{ truncate(displaySubtitle(a), 38) || '—' }}</div></td>
            <td>
              <span class="media-pill" v-if="(a.blocks || []).length">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 7h8M8 12h8M8 17h5"/></svg>
                {{ (a.blocks || []).length }}
              </span>
            ...
            <td>
              <div class="lang-dots">
                <span class="lang-dot lang-dot--ckb" :class="{ 'lang-dot--filled': a.ckbContent?.title && a.slugCkb }" title="سۆرانی">ک</span>
                <span class="lang-dot lang-dot--kmr" :class="{ 'lang-dot--filled': a.kmrContent?.title && a.slugKmr }" title="کرمانجی">م</span>
              </div>
            </td>
            <td>
            ...
            <td @click.stop>
              <div class="tbl__acts">
                <button class="act act--view" @click="openDetail(a)">
        ...
        <div v-if="delTarget" class="overlay" @click.self="delTarget = null">
          <div class="del-modal">
            ...
            <p class="del-modal__body">
              دەربارەی <strong>«{{ delTarget.ckbContent?.title || delTarget.kmrContent?.title || delTarget.slugCkb }}»</strong><br/>
            ...
            <div v-if="detail" class="pdm" role="dialog">
              ...
              <div class="pdm-content">
                <div class="pdm-info__head">
                  ...
                  <!-- Both slugs displayed clearly in modal -->
                  <div class="pdm-slugs">
                  ...
                  <template v-if="detailLang === 'ckb'">
                    ...
                    <p class="pdm-meta-desc" v-if="detail.ckbContent?.metaDescription">{{ detail.ckbContent.metaDescription }}</p>
                  </template>
                  <template v-else>
                    ...
                    <p class="pdm-meta-desc" v-if="detail.kmrContent?.metaDescription">{{ detail.kmrContent.metaDescription }}</p>
                  </template>
                ...
                <div class="pdm-blocks" v-if="(detail.blocks || []).length">
                  <h3 class="pdm-blocks__title">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 7h8M8 12h8M8 17h5"/></svg>
                  ...
                  <div class="block-preview" v-for="(b, idx) in detail.blocks" :key="idx">
                    ...
                    <div class="block-preview__info">
                      <div class="block-preview__head">
                        <span class="block-preview__type" :class="`block-preview__type--${b.contentType.toLowerCase()}`">{{ blockTypeLabel(b.contentType) }}</span>
                        <span class="block-preview__lang-dot" :class="(b.ckbContent?.title || b.ckbContent?.contentText) ? 'block-preview__lang-dot--ok' : ''">ک</span>
                        <span class="block-preview__lang-dot" :class="(b.kmrContent?.title || b.kmrContent?.contentText) ? 'block-preview__lang-dot--ok' : ''">م</span>
                      </div>
                      <template v-if="detailLang === 'ckb'">
                        <span class="block-preview__title" v-if="b.ckbContent?.title">{{ b.ckbContent.title }}</span>
                        <p class="block-preview__text" v-if="b.ckbContent?.contentText">{{ truncate(b.ckbContent.contentText, 120) }}</p>
                      </template>
                      <template v-else>
                        <span class="block-preview__title" v-if="b.kmrContent?.title">{{ b.kmrContent.title }}</span>
                        <p class="block-preview__text" v-if="b.kmrContent?.contentText">{{ truncate(b.kmrContent.contentText, 120) }}</p>
                      </template>
...
function displayTitle(a) {
  return listLang.value === 'ckb'
    ? (a.ckbContent?.title || a.kmrContent?.title || '')
    : (a.kmrContent?.title || a.ckbContent?.title || '')
}
function altTitle(a) {
  if (listLang.value === 'ckb') {
    const kmr = a.kmrContent?.title || '', ckb = a.ckbContent?.title || ''
    return kmr && kmr !== ckb ? kmr : ''
  }
  const ckb = a.ckbContent?.title || '', kmr = a.kmrContent?.title || ''
  return ckb && ckb !== kmr ? ckb : ''
}
function displaySubtitle(a) {
  return listLang.value === 'ckb'
    ? (a.ckbContent?.subtitle || a.kmrContent?.subtitle || '')
    : (a.kmrContent?.subtitle || a.ckbContent?.subtitle || '')
}

// Search covers all four title fields AND both slugs
const visibleItems = computed(() => {
  let pool = [...items.value]
  if (searchQ.value.trim()) {
    const q = searchQ.value.trim().toLowerCase()
    pool = pool.filter(a =>
      (a.slugCkb                   || '').toLowerCase().includes(q) ||
      (a.slugKmr                   || '').toLowerCase().includes(q) ||
      (a.ckbContent?.title         || '').toLowerCase().includes(q) ||
      (a.ckbContent?.subtitle      || '').toLowerCase().includes(q) ||
      (a.kmrContent?.title         || '').toLowerCase().includes(q) ||
      (a.kmrContent?.subtitle      || '').toLowerCase().includes(q)
    )
  }
...
function openDetail(a) { detail.value = a; detailLang.value = 'ckb' }
...
.alist__sub { margin: 0; color: var(--muted, #767571); font-weight: 600; }
.alist__bar { display: flex; gap: .75rem; margin-bottom: 1rem; flex-wrap: wrap; align-items: center; }
.alist__bar .search { flex: 1; min-width: 200px; }
...
/* Dual slug pills in table */
.slug-stack { display: flex; flex-direction: column; gap: .25rem; }
.slug-pill { display: inline-flex; align-items: center; gap: .3rem; padding: .18rem .5rem; border-radius: 7px; font-family: monospace; font-size: .77rem; font-weight: 600; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.block-preview { display: flex; gap: .75rem; padding: .75rem; border: 1px solid #f0eeeb; border-radius: 12px; margin-bottom: .5rem; }
...
.block-preview__head { display: flex; align-items: center; gap: .35rem; margin-bottom: .3rem; }
.block-preview__type { display: inline-block; padding: .12rem .5rem; border-radius: 6px; font-weight: 700; font-size: .75rem; }
...
.block-preview__lang-dot { display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; border-radius: 5px; font-size: .62rem; font-weight: 800; background: #f0eeeb; color: #aaa; }
.block-preview__lang-dot--ok { background: #ecfdf5; color: #065f46; }
