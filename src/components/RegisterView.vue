                    ...
                    <div v-if="form.password && !v$.password.$error" class="password-strength">
                      <div class="strength-bar">
                        <div 
                          ...
                          :class="`strength-bar__fill--${passwordStrength.level}`"
...
/**
 * ✅ Fix: confirm password validator
 */
...
const rules = {
  ...
  email: {
    required: helpers.withMessage('ئیمەیڵ پێویستە', required),
    email: helpers.withMessage('ئیمەیڵ دروست نیە', emailRule),
  },
  password: {
  ...
  confirm: {
    required: helpers.withMessage('دووبارەکردنەوە پێویستە', required),
    samePassword,
  },
}
...
const submit = async () => {
  ...
  const res = await authStore.registerGuest(payload)
  if (!res.ok) return
...
.brand-panel__inner{
  ...
  padding:3.25rem 3rem; gap:2.2rem; position:relative; z-index:1;
}
...
.feature-item{
  ...
  color:rgba(255,255,255,0.85); font-family:var(--font-ar);
...
.stamp__ring{
  ...
  background:
    radial-gradient(circle at 30% 30%, rgba(254,221,0,0.10), transparent 55%),
...
.stamp__dot{
  ...
  background:rgba(254,221,0,0.55); top:32px; left:32px;
  filter: drop-shadow(0 10px 18px rgba(0,0,0,0.25));
}
.stamp__dot--b{
  top:auto; left:auto; bottom:34px; right:34px;
...
.auth-card__header{
  padding:2.1rem 2rem 1.35rem; text-align:center;
...
.dot.active{
  ...
  box-shadow:0 0 0 3px rgba(140,21,21,0.15);
}
...
.field__icon{
  ...
  color:var(--g400); font-weight:900; opacity:0.9;
...
.field__input{
  width:100%;
  padding:0.98rem 2.65rem 0.98rem 2.9rem;
  border-radius:14px;
...
.strength-bar__fill--weak{
  background:var(--err);
  box-shadow:0 0 8px rgba(204,41,54,0.4);
}
