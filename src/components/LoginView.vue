...
const handleSubmit = async () => {
  ...
  setTimeout(() => {
    ...
    // ✅ FIXED: Check if user has admin access (EMPLOYEE, ADMIN, or SUPER_ADMIN)
...
.brand-panel__desc{
  ...
  color:rgba(255,255,255,0.78); font-family:var(--font-ar);
  font-size:0.95rem;
}
...
.form-panel__mobile-logo{
  font-family:var(--font-ar); font-weight:1000;
  color:var(--brand); font-size:1.15rem;
}
.form-panel__mobile-sub{
  color:var(--g600); font-weight:800;
  letter-spacing:0.06em; text-transform:uppercase; font-size:0.78rem;
}
.form-panel__version{
  margin:0; font-size:0.75rem; color:var(--g400); direction:ltr;
}
...
.auth-card__title{
  margin:0 0 0.25rem; font-family:var(--font-ar);
  font-weight:1000; font-size:1.75rem; color:var(--g900);
}
.auth-card__subtitle{
  margin:0; color:var(--g600); font-weight:800; font-size:0.9rem;
}
...
.status-text{
  font-size:0.8rem; color:var(--g600); font-weight:800;
}
...
.auth-error__close{
  ...
  font-size:1.25rem; cursor:pointer; opacity:0.75;
...
.success-slide-enter-active,.success-slide-leave-active{
  transition:all .25s ease;
}
.error-slide-enter-from,.error-slide-leave-to,
.success-slide-enter-from,.success-slide-leave-to{
...
.field__label{
  font-family:var(--font-ar); font-weight:900;
  color:var(--g900); font-size:0.85rem;
...
.field__input:disabled{
  opacity:0.75; cursor:not-allowed; background:var(--g50);
}

.field__eye{
  position:absolute; left:0.75rem;
  border:none; background:none; cursor:pointer;
  color:var(--g400); display:flex; padding:0.35rem;
...
.field__error{
  color:var(--err); font-weight:900; font-size:0.75rem;
...
.auth-btn:hover:not(:disabled){
  ...
  box-shadow:0 20px 50px rgba(140,21,21,0.32);
...
.auth-btn:focus-visible{
  outline:none;
  box-shadow:0 0 0 3px rgba(254,221,0,0.25), 0 18px 42px rgba(140,21,21,0.30);
}
.auth-btn:disabled{
  opacity:0.65; cursor:not-allowed;
  transform:none;
  box-shadow:0 8px 20px rgba(140,21,21,0.15);
}
...
.auth-btn:hover:not(:disabled) .auth-btn__icon{
...
.auth-card__footer{
  ...
  border-top:1px solid rgba(240,239,237,0.9);
  display:flex;
  justify-content:space-between;
  gap:1rem;
}
.link-muted,.link-strong{
  text-decoration:none; font-weight:1000; font-size:0.85rem;
  transition:all var(--t);
